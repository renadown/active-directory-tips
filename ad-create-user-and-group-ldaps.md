...
Requisitos:
- URL do Servidor: Endereço de acesso ao servidor LDAPs/AD.
- Diretório Base de Pesquisa as Entradas LDAPs/AD: Nome do BASE-DC (Base Domain
...
```
```bash 
Linux: ldapsearch -x -H ldap://nome-do-servidor -b "dc=example,dc=com" -D "cn=admin,dc=example,dc=com" -W "(cn=ExampleCorpUser)" dn
```
